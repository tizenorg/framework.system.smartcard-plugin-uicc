...
static void _uiccTransmitCallback_sync(TapiHandle *handle, int result,
	void *data, void *user_data)
{
	TelSimAccessResult_t access_rt = (TelSimAccessResult_t)result;
    TelSimApduResp_t *resp_apdu = (TelSimApduResp_t *)data;
	context_s* context = (context_s*)user_data;

	if(access_rt == TAPI_SIM_ACCESS_SUCCESS)
	{
		context->resp.assign(resp_apdu->apdu_resp, resp_apdu->apdu_resp_len);

		_ERR("resp_apdu resp len : %d", resp_apdu->apdu_resp_len);
	}

	g_main_loop_quit(context->loop);
}

static void _uiccGetATRCallback_sync(TapiHandle *handle, int result,
	void *data, void *user_data)
{
	TelSimAccessResult_t access_rt = (TelSimAccessResult_t)result;
	TelSimAtrResp_t *atr  = (TelSimAtrResp_t *)data;
	...
	if(access_rt == TAPI_SIM_ACCESS_SUCCESS)
	{
		context->resp.assign(atr->atr_resp, atr->atr_resp_len);

		_ERR("Get ATR resp len : %d\n", atr->atr_resp_len);
	}
...
//			cpList = tel_get_cp_name_list();
		...
		ByteArray &response)
	{
		...
		SCOPE_LOCK(mutex)
		{
			if (command.size() > 0)
			{
				context_s context = { NULL, response };
				...
				result = tel_req_sim_apdu(handle, &apdu_data,
					_uiccTransmitCallback_sync, &context);
				if (result != 0)
				{
	...
	int UICCTerminal::getATRSync(ByteArray &atr)
	{
		...
		SCOPE_LOCK(mutex)
		{
			context_s context = { NULL, atr };

			context.loop = g_main_loop_new(NULL, false);

			result = tel_req_sim_atr(handle,
				_uiccGetATRCallback_sync, &context);
			if(result != 0)
			{
